...
- 重新設計 Streamlit UI，改善深色模式下的可讀性與版面層次。
- 側邊欄重新整理為會議設定、音訊設定、模式切換與診斷資訊，長文字不再容易被截斷。
- 新增上方中文閱讀流面板，將翻譯結果依語音順序由上到下接續顯示，並自動捲動到最新內容。
- 補強錄音診斷資訊，現在可直接看到已捕捉片段、靜音片段、送往 API 的片段與最近 RMS。
- 強化控制列與狀態卡，讓開始 / 停止錄音與目前系統狀態更容易判讀。
